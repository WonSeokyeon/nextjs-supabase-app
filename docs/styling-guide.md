...
> ⚠️ 실제 설치 버전은 `tailwindcss@^3.4.1`이며 `tailwind.config.ts` + `@tailwind base/components/utilities` 지시어 방식을 사용한다 (v4의 CSS-only 설정 방식이 아님). 아래 예시들도 v3 문법 기준이다.

## 🎨 기술 스택 개요

### 핵심 스타일링 도구

- **TailwindCSS v3**: 유틸리티 기반 CSS 프레임워크 (`tailwind.config.ts` 기반 설정)
- **shadcn/ui**: Radix UI 기반 컴포넌트 라이브러리 (new-york style)
- **next-themes**: 다크모드 지원
- **tailwindcss-animate**: 애니메이션 플러그인 (`tailwind.config.ts`의 `plugins`에 등록됨)
- **CSS Variables**: 동적 테마 시스템

`package.json`에 `prettier`는 설치되어 있지 않다. `prettier-plugin-tailwindcss`로 클래스를 자동 정렬하는 워크플로우는 이 프로젝트에 아직 적용되어 있지 않으므로, 클래스 순서는 아래 가이드를 참고해 수동으로 맞춘다.
...
이 프로젝트에는 자동 정렬 플러그인이 없으므로, 다음 순서를 수동으로 따르세요:
...
이 프로젝트는 `components.json`에서 `baseColor: "neutral"`로 초기화되어 있어, 아래처럼 색조(hue)가 없는 순수 그레이스케일 팔레트를 쓴다 (파란빛이 도는 slate/zinc 계열이 아님):
...
전체 변수(다크모드 포함)는 `app/globals.css`가 원본이므로, 최신 값이 필요하면 그 파일을 직접 확인할 것.
