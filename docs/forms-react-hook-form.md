...
> ⚠️ `react-hook-form`, `zod`, `@hookform/resolvers`는 `package.json`에 **아직 설치되어 있지 않다**. 현재 폼(`components/login-form.tsx` 등)은 모두 순수 `useState` 기반이다. 아래 예시가 사용하는 `@/components/ui/form`, `textarea`, `select` 같은 shadcn/ui 컴포넌트도 `components/ui/`에 아직 없다 (`npx shadcn@latest add form textarea select`로 추가 필요). 이 패턴을 실제로 적용하려면 먼저 패키지와 컴포넌트를 설치해야 한다.
